...
def build_rule_synthesis_prompt(
    ...
    返回 (system, user) 双消息：system = 角色 + 任务 + 产出要求 + 输出格式，user = 已生效规则 + 基座模板 + 修改意见。
    ...
    system = f"""你是资深提示词工程专家。以下是「{scheme_label}」题材在「{stage_desc}」阶段，人类历次打回 LLM 生成结果时写下的修改意见。
