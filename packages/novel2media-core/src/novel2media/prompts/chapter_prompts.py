...
def build_adapt_script_prompt(chapter_text: str, characters_profile: dict, feedback: str = "") -> str:
    ...
    - speaker：本条口播的说话者。旁白写"旁白"；角色对白写对应角色名（已知角色用 profile 名，新角色用原文中文名）。
    ...
    return f"""你是一个专业的有声漫剧单播脚本师，专门制作短视频有声漫剧解说脚本。
...
2. 格式：按剧情顺序逐条分段，一句话一条口播。旁白句严格 10-25 字；角色对白句 1-25 字，可短至一两个字，必须口语化、有冲击力。全部短句，节奏紧凑。
3. 内容规则：删除多余景物描写、无效铺垫、人物内心心理活动。保留核心动作、对话、冲突、神态、主线剧情，全程强化紧张感与悬念。
4. 结构规范：采用短视频爆款结构，开篇高能留人，中段平稳叙事推进剧情，尾段放大冲突，最后一句必须设置下集悬念钩子。
5. 风格要求：百分百口语化、通俗顺口，适配 AI 配音、漫画动态视频，杜绝书面文学腔、不拖沓、不水字。允许保留少量有助于画面感和情绪张力的细节，让口播与画面都有质感，不要过度精简到只剩骨架。
6. 开篇爆款特殊规则：全文改写完成后，摘取本章最高冲突、最惊悚高能的两句话倒叙前置放在开头。先用巅峰画面和冲突三秒锁客，再正常从头顺叙讲解完整剧情，只爆高能看点，不提前剧透结局。
7. 对话处理：凡是有情绪、冲突、转折的角色对话，尽量保留为角色对白，让漫剧有"声临其境"感。角色对白在 text 中直接引用简短台词，不要加"某某说/道/怒喝"等叙述前缀；由画面（角色、表情、动作）告诉用户是谁在说话。每条对白独立成条，speaker 标注说话角色。其余剧情推进用旁白，不要整段对白堆砌。
8. speaker 标记：每条必须标注说话者。旁白写"旁白"；角色对白写对应角色名（已知角色用 profile 名，新角色用原文中文名）。即使当前统一音色，也必须保留，用于后续分轨。
9. 画面与口播的对应关系：本阶段只拆口播条目，不决定分镜。一个画面可以持续多条口播，画面切换由后续分镜节点根据 scene_change 判断。
...
- speaker：本条口播的说话者，"旁白"或角色名（已知角色用 profile 名，新角色用原文中文名）。
