...
_TRI_VIEW_PROMPT_RULE = """6. tri_view_prompt（三视图英文提示词）：用于生成角色三视图（从头到脚的全身照，必须包含到脚），必须包含 Japanese anime style, anime art style, cel shading, cel shaded（日系动漫画风、赛璐璐风格）、character turnaround sheet、full body, head to toe（从头到脚全身）、front view / side view / back view、detailed face, highly detailed facial features（面部精细）、consistent outfit / hairstyle / footwear / body shape（三视角服装/发型/鞋子/体型完全一致）、plain white background（白色空白背景）、masterpiece, best quality, ultra detailed, highres（画质词）；并把 appearance 的鲜明特征翻译成英文关键词写入。基于 appearance 推导。
   ...
   - 成年女性「高级暴露」审美（仅 18 岁及以上；未成年 / 少年少女 / 儿童一律保守着装、禁止任何性化）：定位适合性感的成年女性角色，可走碧蓝航线式高级暴露——curvy / voluptuous figure、elegant sheer black gauze / lace layering / tasteful revealing dress、alluring but tasteful，用高透黑纱、蕾丝叠层、修身礼裙、露背 / 侧开衩等优雅方式表现身材，重意境不重裸露；严禁 nude / topless / exposed nipples / exposed genitals / explicit 等露点或情色词（既是底线也过不了审）。性感为可选项，按角色定位决定；非性感定位或未成年角色不写。
...
def build_parse_initial_characters_prompt(character_profiles: str, worldview: str, feedback: str = "") -> tuple[str, str]:
    """构造初始角色解析提示词。

    返回 (system, user) 双消息：system = 角色 + 提取规则 + 输出格式，user = 世界观 + 修改意见 + 角色设定文本。
    ...
    system = f"""你是一个小说角色分析师。从下面的角色设定文本中，提取其中提到的角色（主要角色和龙套都要）。
