...
> **章节合并分组**：主图 init 阶段在 `load_config` 与 `parse_characters_llm` 之间还有一个 interrupt 节点 `configure_chapter_grouping`，让用户选「连续 N 章合并为一组」做剧本化（组 = 新原子单元，透明复用原 chapter_id 契约）。分组三字段经 `_SHARED_FIELDS` 传播到 `plan_graph`。设计与 id 约定见 [`chapter-grouping.md`](./chapter-grouping.md)。
